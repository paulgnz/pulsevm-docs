...
- **Proven execution model.** PulseVM implements the Antelope model (formerly EOSIO) running [XPR Network](https://xprnetwork.org), WAX, and Telos in production, and carries core components — the chainbase state database and the libfc crypto/serialization layer — directly from the reference implementation. The semantics are not new.
